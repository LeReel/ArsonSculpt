...
// Includes for FileExplorer integration
#include <Windows.h>
...
void mouse_callback(GLFWwindow* _window, double _xPos, double _yPos)
{
    ...
    if (altPressed)
    {
        //TODO: find compute algo to rotate around center point
    }
...
void AS_Application::ProcessInput(GLFWwindow* _window)
{
    ...
    altPressed = glfwGetKey(window, GLFW_KEY_LEFT_ALT);
}
...
void AS_Application::MainLoop(AS_Shader _shader, std::vector<AS_Model> _models)
{
    ...
    {
        OpenFileExplorer();
        std::cout << "Selected File: " << m_SelectedFile << " at path: " << m_FilePath << std::endl;
...
void AS_Application::OpenFileExplorer()
{
    ...
    f_SysHr = CoCreateInstance(CLSID_FileOpenDialog, NULL, CLSCTX_ALL, IID_IFileOpenDialog,
    ...
    m_SelectedFile = m_FilePath.substr(slash + 1);

    AS_Model _newModel = AS_Model(m_FilePath);

    m_models.push_back(_newModel);
...
void AS_Application::RenderUI()
{
    ...
    //UI code goes here
    openFileExplorer = ImGui::Button("File Explorer", ImVec2(150, 150));
